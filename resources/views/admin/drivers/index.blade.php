...
<div class="d-flex justify-content-between align-items-center mb-3">
    <div class="d-flex gap-2">
        <a href="{{ route('admin.drivers.create') }}" class="btn btn-success btn-sm"><i class="bi bi-plus-lg"></i> {{ __('Tambah Driver') }}</a>
        @if(request('search'))
            <a href="{{ route('admin.drivers.index') }}" class="btn btn-outline-secondary btn-sm"><i class="bi bi-x-lg"></i></a>
        @endif
    </div>
    <form method="GET" class="d-flex gap-2">
        <div class="input-group">
            <span class="input-group-text bg-white border-end-0"><i class="bi bi-search text-muted"></i></span>
            <input type="text" name="search" class="form-control border-start-0 ps-0" style="min-width:200px;" placeholder="{{ __('Cari nama, email, plat nomor...') }}" value="{{ request('search') }}">
        </div>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                ...
                <form method="POST" action="{{ route('admin.drivers.destroy', $driver) }}" class="d-inline" data-confirm="{{ __('Hapus driver?') }}">@csrf @method('DELETE')<button class="btn btn-sm btn-danger"><i class="bi bi-trash"></i></button></form>
            </td>
        </tr>
        @empty
